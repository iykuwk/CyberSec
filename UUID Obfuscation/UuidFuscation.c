...
// UUID Format: 8-4-4-4-12 (32 hex char) 128 bits, 16 bytes of number
char* GenerateUUid(int a, int b, int c, int d, int e, int f, int g, int h, int i, int j, int k, int l, int m, int n, int o, int p) {
	...
	// printf("[i] result: %s\n", (char*)result);
	// usually harmful to directly call result(as it is a dangling pointer) but here not bcoz stack changes(BOOL) are not overwrite on it.
	return (char*)result;
}
