...
/*
parameters: 
	- hThread is the weapon through which context in Windows will eventually execute the queued APC.
...
BOOL RunViaApcInjection(IN HANDLE hThread, IN PBYTE pPayload, IN SIZE_T sPayloadSize) {
	//Here only 2 variables are initialized
	PVOID		pAddress			= NULL;
	DWORD		dwOldProtection		= NULL;

	// pAddress: points to the memory where u copied the payload.(Points to address of calculator)
